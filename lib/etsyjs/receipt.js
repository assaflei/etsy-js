(function() {
  ...
  Receipt = (function() {
    ...
    Receipt.prototype.find = function(shopId, receiptId, cb) {
      return this.client.get("/shops/" + shopId + "/receipts/" + receiptId, function(err, status, body, headers) {
        if (err) {
          return cb(err);
        }
        if (status !== 200) {
    ...
    Receipt.prototype.updateShipping = function(shopId, recieptId, trackingData, cb) {
      return this.client.post("/shops/" + shopId + "/receipts/" + recieptId + "/tracking", trackingData, null, function(err, status, body, headers) {
        if (err) {
          return cb(err);
        }
        if (status !== 200) {
          return cb(new Error("updateShipping error: status: " + status + ", error: " + body.error));
        } else {
          return cb(null, body, headers);
        }
      });
