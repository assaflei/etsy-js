(function() {
  ...
  User = (function() {
    ...
    User.prototype.shops = function(cb) {
      return this.client.get("/users/" + this.userId + "/shops", function(err, status, body, headers) {
        if (err) {
          return cb(err);
        }
        if (status !== 200) {
          if (status === 400) {
            return cb(new Error("Get user shops error error: " + body.error));
          } else if (status === 404) {
            return cb(null, {
              results: []
            }, headers);
          } else {
            return cb(new Error("Get user shops error. Status: " + status + ", message: " + body.error));
          }
        } else {
          return cb(null, body, headers);
        }
    ...
    User.prototype.shippingTemplates = function() {
      var cb, params, _i, _ref;
      params = 2 <= arguments.length ? __slice.call(arguments, 0, _i = arguments.length - 1) : (_i = 0, []), cb = arguments[_i++];
      return (_ref = this.client).get.apply(_ref, ["/users/" + this.userId + "/shipping/templates"].concat(__slice.call(params), [function(err, status, body, headers) {
        if (err) {
          return cb(err);
        }
        if (status !== 200) {
