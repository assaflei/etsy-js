(function() {
  ...
  Shop = (function() {
    ...
    Shop.prototype.activeListings = function() {
      ...
      return (_ref = this.client).get.apply(_ref, ["/shops/" + this.shopId + "/listings"].concat(__slice.call(params), [token], [secret], [function(err, status, body, headers) {
        if (err) {
          return cb(err);
        }
        if (status !== 200) {
    ...
    Shop.prototype.soldOutListings = function() {
      var cb, limit, offset, params, secret, token, _arg, _i, _ref;
      params = 3 <= arguments.length ? __slice.call(arguments, 0, _i = arguments.length - 2) : (_i = 0, []), _arg = arguments[_i++], cb = arguments[_i++];
      token = _arg.token, secret = _arg.secret, limit = _arg.limit, offset = _arg.offset;
      params = params == null ? {} : params;
      ...
      return (_ref = this.client).get.apply(_ref, ["/shops/" + this.shopId + "/listings"].concat(__slice.call(params), [token], [secret], [function(err, status, body, headers) {
        ...
        if (status !== 200) {
          return cb(new Error('Get sold out listings error'));
        } else {
          return cb(null, body, headers);
        }
      }]));
    ...
    Shop.prototype.shippingProfiles = function(cb) {
      return this.client.get("/shops/" + this.shopId + "/shipping-profiles", function(err, status, body, headers) {
        if (err) {
          return cb(err);
        }
        if (status !== 200) {
          return cb(new Error('Get shipping template error'));
        } else {
          return cb(null, body, headers);
        }
      });
    ...
    Shop.prototype.updateShippingProfile = function(profileId, details, cb) {
      return this.client.put("/shops/" + this.shopId + "/shipping-profiles/" + profileId, JSON.stringify(details), "application/json", function(err, status, body, headers) {
        if (err) {
          return cb(err);
        }
        if (status !== 200) {
    ...
    Shop.prototype.processingProfiles = function(cb) {
      return this.client.get("/shops/" + this.shopId + "/readiness-state-definitions", function(err, status, body, headers) {
        if (err) {
          return cb(err);
        }
        if (status !== 200) {
          return cb(new Error('Get processing profiles error'));
        } else {
          return cb(null, body, headers);
        }
      });
